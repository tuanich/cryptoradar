...
function analyze(closes, vols = []) {
  ...
  // Only closes + vols are available here (no high/low), so each candle's
  // volume is classified buy/sell by its close-to-close direction over a
  // recent window. This is an ESTIMATE of pressure, not real taker order-flow
  // (that needs Binance taker data, which is geo-blocked on Vercel).
  ...
  for (let i = closes.length - win; i < closes.length; i++) {
    ...
    if (d > 0) buyVol += v;
    else if (d < 0) sellVol += v;
    else { buyVol += v / 2; sellVol += v / 2; }
  }
  ...
  return { overall,score,
    indicators:{maSig,rsiSig,macdSig,srSig,volSig,volBuyPct,volBuy:buyVol,volSell:sellVol},
